...
func (ps *PsqlStorage) Add(ctx context.Context, blacklist bool, ip string) error {
	// Check IP
	if ip == "" {
...
func (ps *PsqlStorage) Delete(ctx context.Context, blacklist bool, ip string) error {
	// Check IP
	if ip == "" {
		return errors.ErrEmptyIP
	}
